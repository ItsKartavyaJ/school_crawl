...
def run_pipeline(
    ...
    Full pipeline for one school:
      ...
      4. Chunk entities
      5. Embed chunks (Gemini) — batched
      6. Upload to Qdrant Cloud
      7. Export Excel + JSON
    """
    ...
    crawl_result: SchoolCrawlResult = crawl_school(
        url=url, school_name=school_name, crawl_dir=crawl_dir
    )
    ...
    _save_checkpoint(
        [{"type": e.entity_type, "text": e.text, "attrs": e.attributes,
          "source": e.source_url, "page": e.source_page} for e in all_entities],
    ...
    # ── Step 5: Embed ─────────────────────────────────────────────────────────
    t0 = time.perf_counter()
    logger.info("Step 4/7: Embedding entities...")
    ...
    if raw_chunks:
        # Embed raw chunks using the same embedder
